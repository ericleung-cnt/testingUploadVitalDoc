...
public class RatingJpaDao extends AbstractJpaDao<Rating, CommonPK> implements IRatingDao {
	 ...
	 */
	@Override
	public Map<String, Long> countAge(Date startDate, Date endDate, String partType){
		StringBuffer sb = new StringBuffer();
		sb.append("select r.engDesc as RANK, count(sr.seafarerId) as COUNT from Rating sr join sr.seafarer s join sr.rank r ");
		sb.append("where s.birthDate between :startDate AND :endDate ");
		if(partType!=null && !partType.isEmpty()){
			sb.append(" and s.partType=:partType");
		}
		sb.append(" group by r.engDesc ");
		...
		Map<String, Long> resultMap = resultList.stream().collect(Collectors.toMap(o->(String)o[0], o->(Long)o[1]));
		return resultMap;
	}
	...
	public Map<String, Long> countAgeWaitingEmployment(Date startDate, Date endDate, String partType){
		StringBuffer sb = new StringBuffer();
//		sb.append(" SELECT SR.RATING AS RANK, COUNT(DISTINCT SR.SEAFARER_ID) AS COUNT FROM SEAFARER_RATING SR ");
//		sb.append(" LEFT JOIN SEAFARER_EMPLOYMENT SE ON SR.SEAFARER_ID = SE.SEAFARER_ID ");
//		sb.append(" LEFT JOIN SEAFARER S ON SR.SEAFARER_ID = S.SEAFARER_ID ");
//		sb.append(" WHERE SE.SEAFARER_ID IS NULL ");
//		sb.append(" AND s.BIRTH_DATE between :startDate AND :endDate ");

//		SELECT SE.CAPACITY AS RANK, COUNT(DISTINCT SE.SEAFARER_ID) AS COUNT FROM SEA_SERVICE SE
//		LEFT JOIN SEAFARER S ON SE.SEAFARER_ID = S.SEAFARER_ID
//		WHERE NOT EXISTS (SELECT 1 FROM SEA_SERVICE SS WHERE SE.SEAFARER_ID = SS.SEAFARER_ID AND SS.DISCHARGE_DATE IS NULL)
//		GROUP BY SE.CAPACITY

		sb.append(" SELECT SE.CAPACITY AS RANK, COUNT(DISTINCT SE.SEAFARER_ID) AS COUNT FROM SEA_SERVICE SE ");
		sb.append(" LEFT JOIN SEAFARER S ON SE.SEAFARER_ID = S.SEAFARER_ID ");
		//sb.append(" LEFT JOIN SEAFARER S ON SR.SEAFARER_ID = S.SEAFARER_ID ");
		sb.append(" WHERE NOT EXISTS (SELECT 1 FROM SEA_SERVICE SS WHERE SE.SEAFARER_ID = SS.SEAFARER_ID AND SS.DISCHARGE_DATE IS NULL)");
		sb.append(" AND s.BIRTH_DATE between :startDate AND :endDate ");
		if(partType!=null && !partType.isEmpty()){
			sb.append(" AND s.PART_TYPE=:partType");
		}
		sb.append(" GROUP BY SE.CAPACITY ");
