...
//		list.add(new PredicateCriteria("itemNo", PredicateType.EQUAL));
	...
	public List<DemandNoteRefund> findByDemandNoteNo(String demandNoteNo){
		Query q = em.createQuery("SELECT distinct dnr from DemandNoteRefund dnr where dnr.dnDemandNoteNo=:demandNoteNo order by dnr.refundId");
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<DemandNoteRefund> findRefund(Date from, Date to){
