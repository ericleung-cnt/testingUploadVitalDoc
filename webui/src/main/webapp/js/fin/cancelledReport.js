var sectionTitle =
	isc.Label.create({
		...
		valign: "top",
		wrap: false,
		contents: "<p><b><font size=2px>Cancelled / Written-off Invoice Report</font></b></p>"
	});
	...
	isc.ReportToolbar.create({
		buttons: [
			{ name:"generateBtn", title:"Generate Report", autoFit: true, disabled: false,
			  click : function () {
				  if(RPT_FIN_Cancelled_Form.validate()){
						ReportViewWindow.displayReport(["RPT_FIN_CANCELLED", RPT_FIN_Cancelled_Form.getValues()]);
					}
			  }
			}
		]
