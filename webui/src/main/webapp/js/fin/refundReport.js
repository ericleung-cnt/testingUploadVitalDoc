var sectionTitle = 	
	isc.Label.create({
		...
		valign: "top",
		wrap: false,
		contents: "<p><b><font size=2px>Refund Report</font></b></p>"
	});
...
var searchForm = 
	isc.ReportDynamicForm.create({
		...
		fields: [
					...
					{name: "receiptDate", title: "Receipt Date",	type: "date", required:false, displayFormat:"dd/MM/yyyy", endRow: true}
				]
	});
	...
	isc.ReportToolbar.create({
		buttons: [
			{ name:"generateBtn", title:"Generate Report", autoFit: true, disabled: false,
			  click : function () { 
				  if(RPT_FIN_Refund_Form.validate()){
						ReportViewWindow.displayReport(["RefundReportUI", RPT_FIN_Refund_Form.getValues()]);
					}
			  }
			}
		]
