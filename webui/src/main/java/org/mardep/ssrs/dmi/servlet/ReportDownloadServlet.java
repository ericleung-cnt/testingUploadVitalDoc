...
public class ReportDownloadServlet implements HttpRequestHandler {
	...
	private final Logger logger = LoggerFactory.getLogger(ReportDownloadServlet.class);

	private final static String SESSION_ATT_REPORT_PARAM = "reportParam";
	private final static String KEY_CONTENT_DISPOSITION = "Content-Disposition";
	private final static String DEFAULT_CONTENT_DISPOSITION = "attachment";
	...
	public void handleRequest(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse)
		...
		String key = (index != -1) ? httpServletRequest.getRequestURI().substring(index + 1) : "";
		HttpSession session = httpServletRequest.getSession();
		User user = (User)session.getAttribute(Key.CURRENT_USER);
		UserContextThreadLocalHolder.setCurrentUser(user);
		Map<String, Object> map = (Map<String, Object>)session.getAttribute(SESSION_ATT_REPORT_PARAM + key);
		session.removeAttribute(SESSION_ATT_REPORT_PARAM + key);
		try {
