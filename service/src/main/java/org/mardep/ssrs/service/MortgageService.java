...
public class MortgageService extends AbstractService implements IMortgageService {
	...
	public Mortgage receiveNewMortgageApplication(Mortgage mortgage) {
		inbox.startWorkflow("registerMortgage", mortgage.getApplNo(), "", "", "");
		return mortgage;
	}
	...
	public Mortgage acceptReg(Mortgage mortgage, List<String> mortgagors, List<Mortgagee> mortgagees, Long taskId) {
		...
		List<Mortgage> list = dao.findByCriteria(mortgage);
		char next;
		if (list.isEmpty()) {
			next = 'A';
		} else {
			list.sort((a,b) ->{ return a.getPriorityCode().compareTo(b.getPriorityCode()); } );
			Mortgage last = list.get(list.size() - 1);
			next = (char) (last.getPriorityCode().charAt(0) + 1);
		}
	...
	public Mortgage approveTransfer(Mortgage mortgage, List<String> mortgagors, List<Mortgagee> mortgagees, Long taskId) {
		changeMortgagees(mortgage, mortgagors, mortgagees, taskId, null, "approveTransferMortgage");
		return mortgage;
	}
	@Override
	...
	@Override
	public Mortgage changeMortgagees(Mortgage mortgage, List<String> mortgagors, List<Mortgagee> mortgagees) {
		return changeMortgagees(mortgage, mortgagors, mortgagees, null, null, null);
	}
	private Mortgage changeMortgagees(Mortgage mortgage, List<String> mortgagors, List<Mortgagee> mortgagees, Long taskId, Transaction tx,
		...
		for (String name : mortgagors) {
			if (!names.contains(name)) {
				...
				mortgagor.setPriorityCode(mortgagor.getPriorityCode());
