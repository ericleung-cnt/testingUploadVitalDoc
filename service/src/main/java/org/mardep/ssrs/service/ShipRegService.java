				...
				if (Boolean.getBoolean("LvpfsService.allInterested") || obj instanceof RegMaster) {
					es.submit(()-> {
						try {
							...
							if (false) lvpfs.send(operation, applNo_);	// resume when lvpfs ready
						} catch (JsonProcessingException e) {
							logger.warn("failed to write {}", e);
						}
					} );
		...
		if (imo!=null) {
			...
			if (rmList!=null && rmList.size()>0) {
				for (RegMaster rm:rmList) {
					...
					if (!rm.getApplNo().equals(regMaster.getApplNo()) && (rm.getRegStatus().equals("A")||rm.getRegStatus().equals("R"))) {
