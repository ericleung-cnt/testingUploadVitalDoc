	...
	public ReceiptStatusResponse processDnsRequest(ReceiptStatusRequest receiptStatusRequest){
		...
		try {
			...
			if (CreateReceiptAction.U.equals(receiptStatusRequest.getCreateReceipt().getAction())){
				...
				BigDecimal receiptReqAmount = req.getReceiptAmount();
				boolean futureReceipt = false;
				if(DateUtils.truncate(receiptDate, Calendar.DATE).compareTo(DateUtils.truncate(new Date(), Calendar.DATE))>0){
					futureReceipt = true;
				...
				demandNoteReceipt.setAmount(receiptReqAmount);
...
//						mailService.send(dnsEmailTemplate, attachments);
				...
				if (demandNoteHeader.getCwStatus() != null){
					...
					logger.info("event={}; resultCode={}; error={};", "Create Receipt", brr.getResultCode().getCode(), brr.getDescription());
					String cwText = "";
					if (demandNoteHeader.getCwStatus().equals(CwIndicator.C.getCode())){
						cwText = "Cancelled";
					} else if (demandNoteHeader.getCwStatus().equals(CwIndicator.W.getCode())){
						cwText = "Written-off";
					}
...
//				mailService.send(dnsEmailTemplate, attachments);
	...
	private DemandNoteHeader updatePaymentStatus(DemandNoteHeader demandNote) {
		...
		if ("1".equals(demandNote.getEbsFlag())) {
			boolean cancelled = false;
			for (DemandNoteReceipt dnr : receipts) {
				if (dnr.getCancelDate() != null) {
					cancelled = true;
					demandNote.setPaymentStatus(null);
					break;
				}
			}
			if (!cancelled) {
				for (DemandNoteReceipt dnr : receipts) {
					if (dnr.getInputTime() != null && 
							dnr.getInputTime().getTime() > System.currentTimeMillis() && 
							dnr.getCancelDate() == null && 
							"70".equals(dnr.getPaymentType())) {
						demandNote.setPaymentStatus(DemandNoteHeader.PAYMENT_STATUS_AUTOPAY_ARRANGED);
						break;
					}
				}
			}
		...
		if (!DemandNoteHeader.PAYMENT_STATUS_AUTOPAY_ARRANGED.equals(demandNote.getPaymentStatus())) {
			...
			for (DemandNoteRefund r : refunds) {
				paid = paid.subtract(r.getRefundAmount());
				demandNote.setStatus(DemandNoteHeader.STATUS_REFUNDED);
			}
			BigDecimal demandNoteAmount = demandNote.getAmount();
			if (paid.compareTo(BigDecimal.ZERO) == 0) {
				demandNote.setPaymentStatus(DemandNoteHeader.PAYMENT_STATUS_OUTSTANDING);
			}else if(paid.compareTo(demandNoteAmount)>0){
