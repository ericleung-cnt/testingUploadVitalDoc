...
public class MMO_001 extends AbstractReportGenerator implements IReportGenerator{
	...
	public byte[] generate(Map<String, Object> inputParam) throws Exception {
		...
		parameter.put("SERB number", "SERB_" + s.getSerbNo());
		byte[] download;
		parameter.put("Type", "Photo-font view");
		download = client.download("MMO-Seafarer Image", parameter);
		if (download.length > 0) {
			logger.info("download front photo {}" ,download.length );
			map.put("front", new ByteArrayInputStream(download));
		}
		parameter.put("Type", "Photo-side view");
		download = client.download("MMO-Seafarer Image", parameter);
		if (download.length > 0) {
			logger.info("download side photo {}" ,download.length );
			map.put("side", new ByteArrayInputStream(download));
		}
		parameter.put("Type", "Fingerprint-left");
		download = client.download("MMO-Seafarer Image", parameter, true);
		if (download.length > 0) {
			logger.info("download left finger {}" ,download.length );
			map.put("left", new ByteArrayInputStream(download));
		}
		parameter.put("Type", "Fingerprint-right");
		download = client.download("MMO-Seafarer Image", parameter, true);
		if (download.length > 0) {
			logger.info("download right finger {}" ,download.length );
			map.put("right", new ByteArrayInputStream(download));
		}
