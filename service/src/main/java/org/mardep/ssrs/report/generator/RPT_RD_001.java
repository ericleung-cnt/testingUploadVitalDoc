...
@Service("RPT_RD_001")
public class RPT_RD_001 extends AbstractSrReport {
	@Autowired
	...
	private Map<String, TranscriptGrantingOffice> getIssueOfficeCount(List<EntityCertIssueLog> certs){
		...
		for (EntityCertIssueLog cert:certs) {			
			...
			if(officeMap.containsKey(cert.getIssueOffice())) {
				...
				//if ("REVISED_COR".equals(cert.getCertType())) {
				if (CertificateTypeEnum.REVISED_COR.toString().equals(cert.getCertType())) {
					trans.setUncertifiedTranscriptCount(trans.getUncertifiedTranscriptCount()+1);
				//} else if ("NEW_COR".equals(cert.getCertType())) {
				} else if (CertificateTypeEnum.NEW_COR.toString().equals(cert.getCertType())) {
